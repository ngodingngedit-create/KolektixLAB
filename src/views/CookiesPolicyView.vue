<template>
  <div class="cookies-page" style="padding-top: 90px;">
    <!-- Hero -->
    ...
    <section class="section">
      <div class="container cookies-layout">
        ...
        <div class="cookies-content">
          ...
          <section id="intro" class="content-section">
            <h2>{{ t('cookies.sections.intro.title') }}</h2>
            <p>{{ t('cookies.sections.intro.text') }}</p>
          </section>
          ...
          <section id="types" class="content-section">
            ...
            <p>{{ t('cookies.sections.types.text') }}</p>
            <ul class="custom-list">
              <li
                v-for="(item, idx) in t('cookies.sections.types.items')"
                :key="idx"
                v-html="item"
              ></li>
            </ul>
          ...
          <section id="how" class="content-section">
            <h2>{{ t('cookies.sections.how.title') }}</h2>
            <p>{{ t('cookies.sections.how.text') }}</p>
            <ul class="custom-list">
              <li
                v-for="(item, idx) in t('cookies.sections.how.items')"
                :key="idx"
              >
                {{ item }}
              </li>
          ...
          <section id="choices" class="content-section">
            <h2>{{ t('cookies.sections.choices.title') }}</h2>
            <p>{{ t('cookies.sections.choices.text') }}</p>
          </section>

          <!-- Section 6: Policy Updates -->
          <section id="updates" class="content-section">
            <h2>{{ t('cookies.sections.updates.title') }}</h2>
            <p>{{ t('cookies.sections.updates.text') }}</p>
          </section>
          ...
          <section id="contact" class="content-section contact-card-section">
            ...
            <div class="contact-details">
              <div class="detail-item">
                <MailIcon :size="16" />
                <span>{{ t('cookies.sections.contact.email') }}</span>
              </div>
              <div class="detail-item">
                <PhoneIcon :size="16" />
                <span>{{ t('cookies.sections.contact.phone') }}</span>
              </div>
              <div class="detail-item">
                <MapPinIcon :size="16" />
                <span>{{ t('cookies.sections.contact.address') }}</span>
              </div>
            </div>
          </section>
        </div>
...
const sidebarItems = computed(() => ({
  intro: t('cookies.sidebar.intro'),
  what: t('cookies.sidebar.what'),
  types: t('cookies.sidebar.types'),
  how: t('cookies.sidebar.how'),
  choices: t('cookies.sidebar.choices'),
  updates: t('cookies.sidebar.updates'),
  contact: t('cookies.sidebar.contact')
}))
